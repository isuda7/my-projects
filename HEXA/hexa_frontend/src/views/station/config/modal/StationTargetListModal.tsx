/**
 * 스테이션 설정 변경 - 대상 스테이션 모달 Component
 * URL: /station/config/management/add - 대상 스테이션
 */
...
export default function StationTargetListModal(props: any) {
	...
	const setInitData = async () => {
		...
		if(resData.data && Array.isArray(resData?.data?.content)) {
			districts = (resData.data.content as StationIdCodeDto[]).map((v: StationIdCodeDto) => ({code: v.districtCode, value: v.districtName}))
		}
	...
	const setInitColumn = (seletData: any[]) => {
		...
		newColumn.forEach(v => {
			...
			if (v.field === 'districtCode') v.selectData = seletData[2];
		})
