/**
 * 함체, 슬롯별 교환횟수 > 함체 Component
 * URL: /station/statistics/device-slot > 함체 탭
 */
...
export default function DeviceExchangeList() {
	...
	const getStationStatDeviceSwapCount = async (params: any) => {
		const result = await StationApiService().getStationStatDeviceSwapCount(params, 'chs');
		setColumnData();
		return result.data;
	}
    ...
    await StationApiService().downloadExcelStationStatDeviceSwapCount(excelParams, 'chs');
	}
